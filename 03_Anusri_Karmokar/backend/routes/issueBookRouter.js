...
router.post('/issue-book', authMiddleware, async (request, response) =>{
    try{
        ...
        return response.status(200).json({message:'Book issued successfully', issueBookRecord});
         ...
         response.status(500).json({message:"Error issuing book".message});
    }
});

router.post('/return-book', authMiddleware, async(request,response)=>{
    try{
        const user = request.user;
        if(user.role !== 'LIBRARIAN'){
        ...
        const issueBook =await IssueRecord.findById(request.params.id);
        if(!issueBook){
